...
def train_resnet_dataloader_representationlearning_simCLR(save_filename,logging_filename,train_dataset_list,val_dataset_list,test_dataset_list,batch_size = 512,entropy_hyperparam=0.,
    ...
    train_dataset = Sleep_Dataset_withPath_sleepEDF_simCLR(dataset_list=train_dataset_list,class_num=class_num,
    use_channel=use_channel,use_cuda = True,preprocessing = True,sample_rate=sample_rate,
                 preprocessing_method = ['permute','crop'],
                 permute_size=sample_rate,
                 crop_size=2500,
                 cutout_size=1000,
    ...
    val_dataset = Sleep_Dataset_withPath_sleepEDF_simCLR(dataset_list=val_dataset_list,class_num=class_num,
    use_channel=use_channel,use_cuda = True,preprocessing = True,sample_rate=sample_rate,
                 preprocessing_method = ['permute','crop'],
                 permute_size=sample_rate,
                 crop_size=2500,
    ...
    loss_fn = SupConLoss(temperature=0.07,contrast_mode='all').to(device)
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_dataloader,desc='Train',unit='batch') as tepoch:
            for index,(batch_signal1,batch_signal2, batch_label) in enumerate(tepoch):

                bsz = batch_signal1.shape[0]

                batch_signal1 = batch_signal1.to(device)
                batch_signal2 = batch_signal2.to(device)
                batch_label = batch_label.long().to(device)
                batch_signal = torch.cat([batch_signal1,batch_signal2],dim=0)
                ...
                # print(features)
                f1, f2 = torch.split(pred, [bsz, bsz], dim=0)
...
def training_resnet_dataloader_representationlearning_simCLR(use_dataset='sleep_edf',total_train_percent = 1.,train_percent=0.8,val_percent=0.1,test_percent=0.1,use_model = 'resnet18',
    ...
    model_save_path = f'/data/hdd3/git/DeepSleepNet_pytorch/saved_model/representation_learning_simCLR/{use_dataset}/{classification_mode}/'\
    ...
    logging_save_path = f'/data/hdd3/git/DeepSleepNet_pytorch/log/representation_learning_simCLR/{use_dataset}/{classification_mode}/'\
    f'single_epoch_models_{round(train_percent,2)}_{round(val_percent,2)}_{round(test_percent,2)}/'\
    f'optim_{optim}_random_seed_{random_seed}_scheduler_{scheduler}_withoutRegularization_aug_p_{aug_p}_aug_method_{aug_method}/'\
    f'firstconv_{first_conv}_maxpool_{maxpool}_layerfilters_{layer_filters}_blockkernelsize_{block_kernel_size}_blockstridesize_{block_stride_size}/'
    ...
    # exit(1)
    train_resnet_dataloader_representationlearning_simCLR(save_filename=save_filename,logging_filename=logging_filename,train_dataset_list=training_fold_list,val_dataset_list=validation_fold_list,test_dataset_list=test_fold_list,
